...
class ReactiveEffect {
  ...
  set dirty(v) {
    this._dirtyLevel = v ? DirtyLevels.Dirty : 0
  }
  run() {
    this._dirtyLevel = 0
    if (!this.active) {
...
function renderSlot(slots, name, props = {}, fallback, noSlotted) {
  ...
  const rendered = createBlock(
    ...
    validSlotContent && slots._ === 1 ? PatchFlags.STABLE_FRAGMENT : -2
  )
...
function createBaseVNode(
  ...
  if (needFullChildrenNormalization) {
    ...
    vnode.shapeFlag |= isString(children) ? ShapeFlags.TEXT_CHILDREN : 16
  }
...
const createApp = (...args) => {
  const a = ensureRenderer()
  console.log(a)
  debugger
  const app = a.createApp(...args)
  {
